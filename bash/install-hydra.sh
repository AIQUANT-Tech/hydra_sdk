...
cat > "$BASH_DIR/generate-keys.sh" << 'GENKEYS'
#!/bin/bash
...
chmod +x "$BASH_DIR/generate-keys.sh"
echo "✓ Created: generate-keys.sh"

# Wallet Balance Script
cat > "$BASH_DIR/wallet-balance.sh" << 'BALANCE'
#!/bin/bash
...
chmod +x "$BASH_DIR/wallet-balance.sh"
echo "✓ Created: wallet-balance.sh"

# Setup Protocol Parameters Script
cat > "$BASH_DIR/setup-protocol-params.sh" << 'PROTOCOL'
#!/bin/bash
...
chmod +x "$BASH_DIR/setup-protocol-params.sh"
echo "✓ Created: setup-protocol-params.sh"

# Start Hydra Node Script
cat > "$BASH_DIR/start-hydra-node.sh" << 'STARTHYDRA'
#!/bin/bash
...
chmod +x "$BASH_DIR/start-hydra-node.sh"
echo "✓ Created: start-hydra-node.sh"

# Connect WebSocket Script
cat > "$BASH_DIR/connect-websocket.sh" << 'WEBSOCKET'
#!/bin/bash
...
chmod +x "$BASH_DIR/connect-websocket.sh"
echo "✓ Created: connect-websocket.sh"

# Create transaction helper
cat > "$BASH_DIR/create-transaction.sh" << 'CREATETX'
#!/bin/bash
...
chmod +x "$BASH_DIR/create-transaction.sh"
echo "✓ Created: create-transaction.sh"

# Commit funds helper
cat > "$BASH_DIR/commit-funds.sh" << 'COMMIT'
#!/bin/bash
...
chmod +x "$BASH_DIR/commit-funds.sh"
...
echo "   ./bash/generate-keys.sh alice"
echo "   ./bash/generate-keys.sh bob"
echo ""
echo "4. Fund addresses from faucet"
echo ""
echo "5. Setup protocol parameters:"
echo "   ./bash/setup-protocol-params.sh"
echo ""
echo "6. Start Hydra nodes:"
echo "   ./bash/start-hydra-node.sh alice bob"
echo "   ./bash/start-hydra-node.sh bob alice"
echo ""
